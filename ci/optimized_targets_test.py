...
class GeneralTestsOptimizerTest(fake_filesystem_unittest.TestCase):
  ...
  def _create_build_context(
    ...
    if not test_context:
      test_context = self._create_test_context()
    build_context_dict = {}
    build_context_dict['enabledBuildFeatures'] = {'optimized_build'}
    if general_tests_optimized:
      build_context_dict['enabledBuildFeatures'].add('general_tests_optimized')
    build_context_dict['testContext'] = test_context
    return BuildContext.create(build_context_dict)
