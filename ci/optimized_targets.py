...
from build_test_suites_utils import *
...
class GeneralTestsOptimizer(OptimizedBuildTarget):
  ...
  def get_build_targets_impl(self) -> set[str]:
    ...
    test_infos = self.build_context.get('testContext', dict()).get(
        'testInfos', []
    )
    test_mapping_test_groups = set()
    for test_info in test_infos:
      is_test_mapping = False
      current_test_mapping_test_groups = set()
      for opt in test_info.get('extraOptions', []):
        if opt.get('key') == 'test-mapping-test-group':
          is_test_mapping = True
          current_test_mapping_test_groups.update(opt.get('values', set()))

      uses_general_tests = build_target_used(
          'general-tests', aggregate_file_download_options(self.build_context)
      )
    ...
    changed_files = self._find_changed_files(change_info_file_path)
  ...
  def _find_changed_files(self, change_info_file_path) -> set[str]:
    try:
  ...
  def _find_affected_modules(
    ...
    # }
    print(test_mappings)
    print(test_mapping_test_groups)
