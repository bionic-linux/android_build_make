...
class GeneralTestsOptimizer(OptimizedBuildTarget):
  ...
  def get_package_outputs_commands_impl(self):
    ...
    # Add host testcases.
    zip_command.extend(
        self._add_items_to_zip(
            prefix='host',
            relative_root=str(os.path.join(src_top, soong_host_out)),
    ...
    zip_command.extend(
        self._add_items_to_zip(
            prefix='target',
            relative_root=str(os.path.join(src_top, product_out)),
    ...
    zip_command.extend(
        self._add_items_to_zip(
            ...
            files=[
                str(os.path.join(framework_path, 'cts-tradefed.jar')),
                str(
                    os.path.join(framework_path, 'compatibility-host-util.jar')
                ),
                str(os.path.join(framework_path, 'vts-tradefed.jar')),
            ],
        )
    )
  ...
  def _get_zip_test_configs_zips_commands(
    ...
    built and general-tests_list.zip which contains a text file which lists
    ...
    Args:
      ...
      host_config_files: list of all host config files. target_config files:
        list of all target config files.

    Returns:
      The commands to generate general_test_list.zip and
      general-tests_configs.zip
    """
    with open(
    ...
    tests_config_zip_command.extend(
        self._add_items_to_zip(
            ...
            list_files=[str(os.path.join(host_out, 'host_general-tests_list'))],
        )
    )
    ...
    tests_list_zip_command.extend(
        self._add_items_to_zip(
            relative_root=str(host_out),
            files=[str(os.path.join(host_out, 'general-tests_list'))],
        )
    )
  ...
  def _query_soong_vars(
    ...
    process_result = subprocess.run(
        args=[
            os.path.join(src_top, 'build/soong/soong_ui.bash'),
            '--dumpvar-mode',
            '--abs',
            soong_vars,
        ],
