...
class OptimizedBuildTarget(ABC):
  ...
  def _base_zip_command(
    ...
    src_top = es.environ.get('TOP', os.getcwd())
    logging.info(f'src_top: {src_top} {self._PREBUILT_SOONG_ZIP_PATH}')
    return [
...
class GeneralTestsOptimizer(OptimizedBuildTarget):
  ...
  def get_package_outputs_commands_impl(self):
    ...
    logging.info(f'host_paths: {host_paths}')
    logging.info(f'target_paths: {target_paths}')
    logging.info(f'host_config_files: {host_config_files}')
    logging.info(f'target_config_files: {target_config_files}')
    zip_commands = []
