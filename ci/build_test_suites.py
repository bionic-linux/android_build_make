...
class BuildPlanner:
  ...
  def create_build_plan(self):
    ...
    if 'optimized_build' not in self.build_context['enabled_build_features']:
      return BuildPlan(self.args.extra_targets, [])
    ...
    for target in self.args.extra_targets:
      ...
      target_optimizer = self.target_optimizations[target](
          self.build_context, self.args
      )
      build_targets.extend(target_optimizer.get_build_targets())
      packaging_functions.append(target_optimizer.package_outputs)
