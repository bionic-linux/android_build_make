...
class TestDiscoveryAgent:
  ...
  _TRADEFED_DISCOVERY_OUTPUT_FILE_NAME = "DISCOVERY_OUTPUT_FILE"
  ...
  def discover_test_zip_regexes(self) -> list[str]:
    ...
    with tempfile.NamedTemporaryFile(mode="w+t", delete=False) as temp_file:
      temp_file.name = self._TRADEFED_DISCOVERY_OUTPUT_FILE_NAME
    checkout_root_dir = os.environ.get("TOP")
    java_args = []
    java_args.append(
        checkout_root_dir + "/prebuilts/jdk/jdk21/linux-x86/bin/java"
    )
    java_args.append("-cp")
    java_args.append(
        self.create_classpath(
            checkout_root_dir + self.tradefed_jar_relevant_files_path
        )
    )
    java_args.append(
    ...
    env = os.environ.copy()
    env.update({"DISCOVERY_OUTPUT_FILE": temp_file.name})
    logging.info(f"Calling test discovery with args: {java_args}")
    try:
      result = subprocess.run(
          args=java_args, env=env, capture_output=True, text=True, check=True
      )
      logging.info(f"Test zip discovery output: {result.stdout}")
    except subprocess.CalledProcessError as e:
      raise TestDiscoveryError(
          "Failed to run test discovery, strout: {result.stdout}, strerr:"
          " {result.stderr}, returncode: {e.returncode}"
      )
    with open(temp_file.name, "r") as f:
      data = json.loads(f.read())
      logging.info(f"Test discovery result file content: {data}")
    if self._TRADEFED_NO_POSSIBLE_TEST_DISCOVERY_KEY in data and data[self._TRADEFED_NO_POSSIBLE_TEST_DISCOVERY_KEY]:
      raise TestDiscoveryError("No possible test discovery")
    if data[self._TRADEFED_TEST_ZIP_REGEXES_LIST_KEY] is None or data[self._TRADEFED_TEST_ZIP_REGEXES_LIST_KEY] is []:
