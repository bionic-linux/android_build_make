...
class TestDiscoveryAgent:
  ...
  def discover_test_zip_regexes(self) -> list[str]:
    ...
    with tempfile.NamedTemporaryFile(mode="w+t", delete=False) as temp_file:
      temp_file.name = "DISCOVERY_OUTPUT_FILE"
    ...
    if "NoPossibleTestDiscovery" in data and data["NoPossibleTestDiscovery"]:
      raise TestDiscoveryError("No possible test discovery")
    if data["TestZipRegexes"] is None or data["TestZipRegexes"] is []:
      raise TestDiscoveryError("No test zip regexes returned")
    return data["TestZipRegexes"]
