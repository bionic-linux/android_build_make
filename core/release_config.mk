...
# These are always created, just mark them PHONY to keep ninja happy.
.PHONY: $(_flags_dir)/$(_base_all_release).pb $(_flags_dir)/$(_base_all_release).textproto $(_flags_dir)/$(_base_all_release).json
        _base_all_release :=
    endif
    _flags_dir:=
    _flags_file:=
