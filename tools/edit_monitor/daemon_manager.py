...
class DaemonManager:
  ...
  def _get_process_memory_percent(self, pid):
    try:
  ...
  def _get_process_cpu_percent(self, pid, interval=1):
    try:
      ...
      return (total_end_time - total_start_time) / (uptime_end - uptime_start)
    except (FileNotFoundError, IndexError, ValueError, IOError) as e:
      logging.exception("Failed to get CPU usage.")
      raise e

  def _get_total_cpu_time(self, pid):
    with open(f"/proc/{str(pid)}/stat", "r") as f:
      stats = f.readline().split()
      # utime is the 14th field in /proc/[pid]/stat
      utime = int(stats[13])
      # stime is the 15th field in /proc/[pid]/stat
      stime = int(stats[14])
      return utime + stime
