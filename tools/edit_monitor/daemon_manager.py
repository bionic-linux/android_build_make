...
DEFAULT_PROCESS_TERMINATION_TIMEOUT = 1


def default_daemon_target():
  """Place holder for the default deamon target."""
  print("default daemon target")


class DaemonManager:
  """Starts and managers the daemon."""
  ...
  def start(self):
    try:
      ...
      logging.error("Failed to start daemon manager with error %s", e)

  def stop(self):
    """Stops the deamon process and remove sthe pidfile."""
  ...
  def _terminate_process(
      self, pid: int, timeout: int = DEFAULT_PROCESS_TERMINATION_TIMEOUT
  ):
    """Terminates a process with given pid.

    It first sents a SIGTERM to the process to allow it for proper
    termination with a timeout. If the process is not terminated within
    the timeout, kill it forcefully.
    """
    try:
      ...
      logging.exception("Failed to kill process %d with error: %s", pid, e)
  ...
  def _remove_pidfile(self):
    try:
      ...
      logging.exception(
          "Failed to remove pidfile %s with error: %s.", self.pid_file_path, e
      )

  def _get_pid_file_path(self, pid_file_dir: pathlib.Path) -> pathlib.Path:
    """Generates the file path to store the pidfile.

    The file path should be like the format "/tmp/edit_monitor/xxxx.lock"
