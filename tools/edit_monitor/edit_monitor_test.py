...
class EditMonitorTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.files_under_monitor_dir = pathlib.Path(self.working_dir.name).joinpath(
        'files'
    )
    self.files_under_monitor_dir.mkdir()
  ...
  def test_log_edit_event_success(self):
    ...
    # Create a file.
    test_file = self.files_under_monitor_dir.joinpath('test.txt')
    test_file.touch()
    ...
    # Move the file.
    test_file_moved = self.files_under_monitor_dir.joinpath('new_test.txt')
    ...
    expected_create_event = edit_event_pb2.EditEvent.SingleEditEvent(
        file_path=str(
            self.files_under_monitor_dir.joinpath('test.txt').resolve()
        ),
    ...
    expected_delete_event = edit_event_pb2.EditEvent.SingleEditEvent(
        file_path=str(
            self.files_under_monitor_dir.joinpath('new_test.txt').resolve()
        ),
  ...
  def test_log_edit_event_fail(self):
    ...
    # Create a file.
    self.files_under_monitor_dir.joinpath('test.txt').touch()
  ...
  def _start_test_edit_monitor_process(
    ...
    p = multiprocessing.Process(
        target=edit_monitor.start_edit_monitor,
        args=(str(self.files_under_monitor_dir.resolve()), cclient),
    )
    p.start()

    # Give some time for the observer to start.
    time.sleep(2)
  ...
  def _get_logged_events(self):
    ...
    records = data.split(b'\x00')  # Split by the delimiter
    for record in records:
      if record:
        events.append(clientanalytics_pb2.LogEvent.FromString(record))
