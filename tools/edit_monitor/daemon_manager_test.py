...
class DaemonManagerTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    # Set the tempdir under the working dir so any temp files created during
    # tests will be cleaned.
    tempfile.tempdir = self.working_dir.name

  def tearDown(self):
    # Clean up any child processes left by the tests.
  ...
  def test_start_success(self):
    ...
    # Verify the expected pid file is created
  ...
  def test_start_failed_to_write_pidfile(self):
    ...
    pid_file_path_dir.mkdir(parents=True, exist_ok=True)
    # Make the directory read-only so write pidfile will fail
