...
def create_arg_parser():
  ...
  parser.add_argument(
      '--force_cleanup',
      action='store_true',
      help='Force stop all edit monitors running in the system.',
  )
...
def term_signal_handler(signal_number, frame):
...
def main(argv: list[str]):
  ...
  dm = daemon_manager.DaemonManager(
      binary_path=argv[0], daemon_target=edit_monitor.start, daemon_args=(args.path, args.dry_run))
  if args.force_cleanup:
    try:
      dm.cleanup()
    except Exception:
      logging.exception('Unexpected exception raised when cleanup daemon.')
    return
