...
class ELFParser(object):
  ...
  def _read_llvm_readobj(cls, elf_file_path, header, llvm_readobj):
    """Run llvm-readobj and parse the output."""
    cmd = [llvm_readobj, '--program-headers', '--dynamic-table', '--dyn-symbols', elf_file_path]
  ...
  def _parse_program_headers(cls, lines_it):
    ...
    for line in lines_it:
      ...
      if line == cls._PROGRAM_HEADERS_END_PATTERN: break
  ...
  def check_max_page_size(self, max_page_size):
    ...
    for alignment in self._file_under_test.alignments:
      if alignment % max_page_size != 0:
        error = True
        self._error(f'Load segment has alignment {alignment} but {max_page_size} required.')
        self._note()
        self._note('Fix suggestions:')
        self._note(f'  use linker flag "-Wl,-z,max-page-size={max_page_size}" when compiling this lib')

    # TODO: may want to show multiple types of errors, and only exit once all are shared
    if error:
      sys.exit(2)
