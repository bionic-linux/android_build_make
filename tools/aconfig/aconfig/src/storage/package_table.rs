...
pub fn create_package_table(container: &str, packages: &[FlagPackage], version: u32) -> Result<PackageTable> {
    // create table
    ...
    let mut node_wrappers: Vec<_> =
        packages.iter().map(|pkg: &FlagPackage<'_>| PackageTableNodeWrapper::new(pkg, num_buckets)).collect();
