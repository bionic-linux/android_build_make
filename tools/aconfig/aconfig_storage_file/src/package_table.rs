...
impl PackageTableNode {
    ...
    pub fn into_bytes(&self, version: u32) -> Vec<u8> {
        match version {
            ...
            _ => Self::into_bytes_v1(self), // Default to v1.
        }
    }
...
impl PackageTable {
    ...
    pub fn into_bytes(&self) -> Vec<u8> {
        [
            ...
            self.nodes.iter().map(|v| v.into_bytes(self.header.version)).collect::<Vec<_>>().concat(),
        ]
        .concat()
    }
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, AconfigStorageError> {
        ...
        let nodes = (0..num_packages)
            .map(|_| {
                let node = PackageTableNode::from_bytes(&bytes[head..], version)?;
                head += node.into_bytes(version).len();
                Ok(node)
            })
...
mod tests {
    ...
    fn test_read_info_bt_versions() {
        let mut table_v1: PackageTable = create_test_package_table();

        // Create a copy. Fake v2 because though we will set the version to v2
        ...
        // Only the version should have changed - otherwise the data should have
        // been maintained.
        table_v1.header.version = 2;
