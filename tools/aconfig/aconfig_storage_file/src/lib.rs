...
pub fn read_u8_from_bytes(buf: &[u8], head: &mut usize) -> Result<u8, AconfigStorageError> {
    let val = u8::from_le_bytes(
        buf.get(*head..*head + 1)
            .ok_or(AconfigStorageError::BytesParseFail(anyhow!("fail to parse u8 from bytes: access out of bounds")))?
            .try_into()
...
pub(crate) fn read_u16_from_bytes(
    ...
    let val = u16::from_le_bytes(
        buf.get(*head..*head + 2)
            .ok_or(AconfigStorageError::BytesParseFail(anyhow!("fail to parse u16 from bytes: access out of bounds")))?
            .try_into()
            .map_err(|errmsg| {
...
pub fn read_u32_from_bytes(buf: &[u8], head: &mut usize) -> Result<u32, AconfigStorageError> {
    let val = u32::from_le_bytes(
        buf.get(*head..*head + 4)
            .ok_or(AconfigStorageError::BytesParseFail(anyhow!("fail to parse u32 from bytes: access out of bounds")))?
            .try_into()
            .map_err(|errmsg| {
...
pub fn read_u64_from_bytes(buf: &[u8], head: &mut usize) -> Result<u64, AconfigStorageError> {
    let val = u64::from_le_bytes(
        buf.get(*head..*head + 8)
            .ok_or(AconfigStorageError::BytesParseFail(anyhow!("fail to parse u64 from bytes: access out of bounds")))?
            .try_into()
            .map_err(|errmsg| {
...
pub(crate) fn read_str_from_bytes(
    ...
    let val = String::from_utf8(
        buf.get(*head..*head + num_bytes)
            .ok_or(AconfigStorageError::BytesParseFail(anyhow!("fail to parse string from bytes: access out of bounds")))?
            .to_vec(),
    )
