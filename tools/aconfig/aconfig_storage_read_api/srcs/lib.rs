...
fn get_package_read_context_java(
    ...
    let package_name: String = env.get_string(package)?.into();
...
>(
    ...
    match get_package_read_context_java(env, file, package) {
        Ok(context_opt) => match context_opt {
            ...
            None => create_java_package_read_context(env, true, String::from(""), false, 0, 0),
        },
        Err(errmsg) => {
            create_java_package_read_context(env, false, format!("{:?}", errmsg), false, 0, 0)
        }
    }
}
...
fn get_flag_read_context_java(
    ...
    let flag_name: String = env.get_string(flag)?.into();
    let buffer_ptr = env.get_direct_buffer_address(file)?;
    let buffer_size = env.get_direct_buffer_capacity(file)?;
    // SAFETY:
    // The safety here is ensured as only non null MemoryMappedBuffer will be passed in,
...
>(
    ...
    match get_flag_read_context_java(env, file, package_id, flag) {
        Ok(context_opt) => match context_opt {
            ...
            None => create_java_flag_read_context(env, true, String::from(""), false, 9999, 0),
        },
        Err(errmsg) => {
            create_java_flag_read_context(env, false, format!("{:?}", errmsg), false, 9999, 0)
        }
    }
}
...
fn get_boolean_flag_value_java(env: JNIEnv, file: JByteBuffer, flag_index: jint) -> Result<bool> {
    let buffer_ptr = env.get_direct_buffer_address(file)?;
    let buffer_size = env.get_direct_buffer_capacity(file)?;
    // SAFETY:
    // The safety here is ensured as only non null MemoryMappedBuffer will be passed in,
...
>(
    ...
    match get_boolean_flag_value_java(env, file, flag_index) {
        Ok(value) => create_java_boolean_flag_value(env, true, String::from(""), value),
        Err(errmsg) => create_java_boolean_flag_value(env, false, format!("{:?}", errmsg), false),
    }
}
