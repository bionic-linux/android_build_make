...
fn get_package_read_context_java(
    ...
    let buffer = unsafe {
        std::slice::from_raw_parts(buffer_ptr, buffer_size)
    };
...
>(
    ...
    match get_boolean_flag_value_java(env, file, flag_index) {
        Ok(value) => {
            create_java_boolean_flag_value(env, true, String::from(""), value)
        },
        Err(errmsg) => {
            create_java_boolean_flag_value(env, false, format!("{:?}", errmsg), false)
        }
    }
}
