...
def add_static_deps_of_file(file_id, file_metadata, doc, built_file2soong_module):
  ...
  for dep_file in static_dep_files:
    if dep_file not in built_file2soong_module:
      # Don't include it and it has been reported in report[ISSUE_NO_MODULE_FOUND_FOR_STATIC_DEP]
      continue
...
class MetadataDb:
  ...
  def reorg(self):
    ...
    self.conn.execute('create index idx_package_licnese on package_license (package, license)')
    self.conn.execute('create index idx_module_licnese on module_license (module, package, license)')
    self.conn.execute('create index idx_module_installed_file on module_installed_file (module_id, module_name, package, installed_file)')
    self.conn.execute('create index idx_module_built_file on module_built_file (module_id, module_name, package, built_file)')
    self.conn.commit()

    if args.debug:
      with sqlite3.connect(os.path.dirname(args.metadata) + '/metadata-debug.db') as c:
        self.conn.backup(c)
...
def main():
  ...
  # Get metadata of all soong modules
  soong_modules = db.get_soong_modules()

  built_file2soong_module = {}
  installed_file2soong_module = {}
  static_dep_files = {}
  for soong_module in soong_modules:
    for f in soong_module['installed_files'].split(' '):
      installed_file2soong_module[f] = soong_module
    for f in soong_module['built_files'].split(' '):
      built_file2soong_module[f] = soong_module
    for f in soong_module['static_dep_files'].split(' '):
      static_dep_files[f] = True
    for f in soong_module['whole_static_dep_files'].split(' '):
      static_dep_files[f] = True

  # Check if a static dep file is built by a module, only soong modules are scanned.
  for f in static_dep_files:
    if f not in built_file2soong_module:
  ...
  for installed_file_metadata in installed_files_metadata:
    if installed_file_metadata['installed_file'] in installed_file2soong_module:
      soong_module = installed_file2soong_module[installed_file_metadata['installed_file']]
      ...
      installed_file_metadata['static_dep_files'] = ''  # TODO: populate its value in make later.
      installed_file_metadata['whole_static_dep_files'] = ''  # TODO: populate its value in make later.

  # Find all transitive static dep files of all installed files
  # TODO: only static dep files of soong modules are considered now, need to include make metadata later.
  q = queue.Queue()
  for installed_file_metadata in installed_files_metadata:
  ...
  installed_static_dep_files = {}
  while not q.empty():
    dep_file = q.get()
    if dep_file in installed_static_dep_files:
      # It has been processed
      continue
    if dep_file not in built_file2soong_module:
      # This should not happen, but check report[ISSUE_NO_MODULE_FOUND_FOR_STATIC_DEP]
      continue
    installed_static_dep_files[dep_file] = True
    module_of_dep_file = built_file2soong_module[dep_file]
    if module_of_dep_file['static_dep_files']:
      for f in module_of_dep_file['static_dep_files'].split(' '):
        if f not in installed_static_dep_files:
          q.put(f)
    if module_of_dep_file['whole_static_dep_files']:
      for f in module_of_dep_file['whole_static_dep_files'].split(' '):
        if f not in installed_static_dep_files:
          q.put(f)
  ...
  for installed_file_metadata in installed_files_metadata:
    ...
    add_static_deps_of_file(file_id, installed_file_metadata, doc, built_file2soong_module)
  ...
  for dep_file in sorted(installed_static_dep_files.keys()):
    ...
    file_metadata.update(built_file2soong_module[dep_file])
    add_package_of_file(file_id, file_metadata, doc, report)

    # Add relationships for static deps of static libraries
    add_static_deps_of_file(file_id, file_metadata, doc, built_file2soong_module)
