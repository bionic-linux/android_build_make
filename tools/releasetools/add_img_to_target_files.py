...
def GetCareMap(which, imgname):
    ...
    assert (
        image_size < unsparsed_image_size
    ), f"AVB footer's original image size {image_size} is larger than or equal to image size on disk {unsparsed_image_size}, this can't happen because a verified image = original image + hash tree data + FEC data + avbfooter."
    assert image_size > 0
    ...
    assert (
        image_blocks >= 0
    ), "blocks for {} must be non-negative, image size: {}".format(which, image_size)
...
class OutputFile(object):
    ...
    def Write(self, compress_type=None):
        if self._output_zip:
            common.ZipWrite(
                self._output_zip, self.name, self._zip_name, compress_type=compress_type
            )
...
def AddSystem(output_zip, recovery_img=None, boot_img=None):
    ...
    board_uses_vendorimage = OPTIONS.info_dict.get("board_uses_vendorimage") == "true"

    if (
        OPTIONS.rebuild_recovery
        and not board_uses_vendorimage
        and recovery_img is not None
        and boot_img is not None
    ):
    ...
    CreateImage(
        OPTIONS.input_tmp, OPTIONS.info_dict, "system", img, block_list=block_list
    )
    return img.name
...
def AddVendor(output_zip, recovery_img=None, boot_img=None):
    ...
    CreateImage(
        OPTIONS.input_tmp, OPTIONS.info_dict, "vendor", img, block_list=block_list
    )
    return img.name
...
def AddProduct(output_zip):
    ...
    block_list = OutputFile(output_zip, OPTIONS.input_tmp, "IMAGES", "product.map")
    CreateImage(
        OPTIONS.input_tmp, OPTIONS.info_dict, "product", img, block_list=block_list
    )
    return img.name
...
def AddSystemExt(output_zip):
    ...
    img = OutputFile(output_zip, OPTIONS.input_tmp, "IMAGES", "system_ext.img")
    if os.path.exists(img.name):
        logger.info("system_ext.img already exists; no need to rebuild...")
        return img.name

    block_list = OutputFile(output_zip, OPTIONS.input_tmp, "IMAGES", "system_ext.map")
    CreateImage(
        OPTIONS.input_tmp, OPTIONS.info_dict, "system_ext", img, block_list=block_list
    )
    return img.name
...
def AddOdm(output_zip):
    ...
    CreateImage(OPTIONS.input_tmp, OPTIONS.info_dict, "odm", img, block_list=block_list)
    return img.name
...
def AddVendorDlkm(output_zip):
    ...
    block_list = OutputFile(output_zip, OPTIONS.input_tmp, "IMAGES", "vendor_dlkm.map")
    CreateImage(
        OPTIONS.input_tmp, OPTIONS.info_dict, "vendor_dlkm", img, block_list=block_list
    )
    return img.name
...
def AddOdmDlkm(output_zip):
    ...
    block_list = OutputFile(output_zip, OPTIONS.input_tmp, "IMAGES", "odm_dlkm.map")
    CreateImage(
        OPTIONS.input_tmp, OPTIONS.info_dict, "odm_dlkm", img, block_list=block_list
    )
    return img.name
...
def AddSystemDlkm(output_zip):
    ...
    block_list = OutputFile(output_zip, OPTIONS.input_tmp, "IMAGES", "system_dlkm.map")
    CreateImage(
        OPTIONS.input_tmp, OPTIONS.info_dict, "system_dlkm", img, block_list=block_list
    )
    return img.name
...
def AddDtbo(output_zip):
    ...
    dtbo_prebuilt_path = os.path.join(OPTIONS.input_tmp, "PREBUILT_IMAGES", "dtbo.img")
...
def AddCustomImages(output_zip, partition_name, image_list):
    ...
    for img_name in image_list:
        ...
        assert os.path.exists(custom_image_prebuilt_path), "Failed to find %s at %s" % (
            img_name,
            custom_image_prebuilt_path,
        )
    ...
    assert os.path.exists(default), "There should be one %s.img" % (partition_name)
    return default
...
def CreateImage(input_dir, info_dict, what, output_file, block_list=None):
    ...
    use_dynamic_size = info_dict.get(
        "use_dynamic_partition_size"
    ) == "true" and what in shlex.split(
...
def AddVBMeta(output_zip, partitions, name, needed_partitions):
    ...
    img = OutputFile(output_zip, OPTIONS.input_tmp, "IMAGES", "{}.img".format(name))
    if os.path.exists(img.name):
...
def CheckAbOtaImages(output_zip, ab_partitions):
    ...
    for partition in ab_partitions:
        ...
        if output_zip:
            ...
            available = (
                images_path in output_zip.namelist()
                or radio_path in output_zip.namelist()
            )
        else:
...
def AddPackRadioImages(output_zip, images):
    ...
    for image in images:
        ...
        assert os.path.exists(img_radio_path), "Failed to find %s at %s" % (
            img_name,
            img_radio_path,
        )
...
def AddSuperSplit(output_zip):
    ...
    if built:
        for dev in OPTIONS.info_dict["super_block_devices"].strip().split():
            img = OutputFile(
                output_zip, OPTIONS.input_tmp, "OTA", "super_" + dev + ".img"
            )
            img.Write()
...
def ReplaceUpdatedFiles(zip_filename, files_list):
    ...
    output_zip = zipfile.ZipFile(
        zip_filename, "a", compression=zipfile.ZIP_DEFLATED, allowZip64=True
    )
...
def HasPartition(partition_name):
    ...
    return (
        os.path.isdir(os.path.join(OPTIONS.input_tmp, partition_name.upper()))
        and OPTIONS.info_dict.get("building_{}_image".format(partition_name)) == "true"
    ) or os.path.exists(
        os.path.join(OPTIONS.input_tmp, "IMAGES", "{}.img".format(partition_name))
    )
...
def AddVbmetaDigest(output_zip):
    ...
    if (
        ...
        vbmeta_digest_txt = os.path.join(OPTIONS.input_tmp, "META", "vbmeta_digest.txt")
...
def AddImagesToTargetFiles(filename):
    ...
    if os.path.isdir(filename):
        ...
        output_zip = zipfile.ZipFile(
            filename, "a", compression=zipfile.ZIP_DEFLATED, allowZip64=True
        )
    ...
    if has_vendor_boot:
        ...
        if vendor_boot_image:
            partitions["vendor_boot"] = os.path.join(
                OPTIONS.input_tmp, "IMAGES", "vendor_boot.img"
            )
    ...
    if has_vendor_kernel_boot:
        ...
        if vendor_kernel_boot_image:
            partitions["vendor_kernel_boot"] = os.path.join(
                OPTIONS.input_tmp, "IMAGES", "vendor_kernel_boot.img"
            )
    ...
    if output_zip:
        ...
        with ThreadPoolExecutor(max_workers=len(add_partition_calls)) as executor:
            for future in [
                executor.submit(add_partition, *call) for call in add_partition_calls
            ]:
                future.result()
    ...
    add_partition("dtbo", OPTIONS.info_dict.get("has_dtbo") == "true", AddDtbo, [])
    add_partition("pvmfw", OPTIONS.info_dict.get("has_pvmfw") == "true", AddPvmfw, [])

    # Custom images.
    custom_partitions = (
        OPTIONS.info_dict.get("custom_images_partition_list", "").strip().split()
    )
    for partition_name in custom_partitions:
        ...
        partitions[partition_name] = AddCustomImages(
            output_zip, partition_name, image_list
        )

    avb_custom_partitions = (
    ...
    for partition_name in avb_custom_partitions:
        ...
        image_list = OPTIONS.info_dict.get(
            "avb_{}_image_list".format(partition_name)
        ).split()
        partitions[partition_name] = AddCustomImages(
            output_zip, partition_name, image_list
        )
    ...
    if OPTIONS.info_dict.get("avb_enable") == "true":
        ...
        if vbmeta_system and set(vbmeta_system.split()).intersection(partitions):
            ...
            vbmeta_partitions = [
                item for item in vbmeta_partitions if item not in vbmeta_system.split()
            ]
        ...
        if vbmeta_vendor and set(vbmeta_vendor.split()).intersection(partitions):
            ...
            vbmeta_partitions = [
                item for item in vbmeta_partitions if item not in vbmeta_vendor.split()
            ]
            vbmeta_partitions.append("vbmeta_vendor")
        custom_avb_partitions = (
            OPTIONS.info_dict.get("avb_custom_vbmeta_images_partition_list", "")
            .strip()
            .split()
        )
        if custom_avb_partitions:
            for avb_part in custom_avb_partitions:
                ...
                included_partitions = (
                    OPTIONS.info_dict.get("avb_vbmeta_{}".format(avb_part), "")
                    .strip()
                    .split()
                )
                assert (
                    included_partitions
                ), "Custom vbmeta partition {0} missing avb_vbmeta_{0} prop".format(
                    avb_part
                )
                banner(partition_name)
                logger.info(
                    "VBMeta partition {} needs {}".format(
    ...
    if OPTIONS.info_dict.get("build_super_partition") == "true":
        if (
            OPTIONS.info_dict.get("build_retrofit_dynamic_partitions_ota_package")
            == "true"
        ):
    ...
    banner("radio")
    ab_partitions_txt = os.path.join(OPTIONS.input_tmp, "META", "ab_partitions.txt")
    if os.path.exists(ab_partitions_txt):
        ...
        AddCareMapForAbOta(
            output_zip if output_zip else output_care_map, ab_partitions, partitions
        )
    ...
    if output_zip:
        ...
        if OPTIONS.replace_updated_files_list:
            ReplaceUpdatedFiles(output_zip.filename, OPTIONS.replace_updated_files_list)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with zipfile.ZipFile(zipfile_path, "r", allowZip64=True) as zfp:
            for zinfo in zfp.filelist:
                if not zinfo.filename.startswith(
                    "IMAGES/"
                ) and not zinfo.filename.startswith("META"):
                    continue
                # Don't try to store userdata.img uncompressed, it's usually huge.
                if zinfo.filename.endswith("userdata.img"):
                    continue
                if (
                    zinfo.compress_size > zinfo.file_size * 0.80
                    and zinfo.compress_type != zipfile.ZIP_STORED
                ):
        ...
        with zipfile.ZipFile(zipfile_path, "a", allowZip64=True) as zfp:
            for entry in entries_to_store:
                zfp.write(
                    os.path.join(tmpdir, entry.filename),
                    entry.filename,
...
def main(argv):
    def option_handler(o, a):
        if o in ("-a", "--add_missing"):
            ...
            raise ValueError(
                "--replace_verity_private_key is no longer supported,"
                " please switch to AVB"
            )
        elif o == "--replace_verity_public_key":
            raise ValueError(
                "--replace_verity_public_key is no longer supported,"
                " please switch to AVB"
            )
        elif o == "--is_signing":
